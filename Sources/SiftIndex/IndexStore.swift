...
public final class IndexStore: Sendable {
    ...
    /// Opening words of the prompt Sift's own knowledge extraction sends. Must stay equal to
    /// `Extractor.instructionMarker`; `IndexStoreMarkerTests` fails the build if they drift.
    /// SiftIndex does not depend on SiftBrain, hence the copy rather than a reference.
    static let extractionMarker = "Extract DURABLE, REUSABLE knowledge"
    ...
    static let userSessionPredicate = """
        ...
        session.firstMessage NOT LIKE '\(extractionMarker)%'))
        """
    ...
    /// "today" counter reflect real work instead of hundreds of machine-made sessions.
