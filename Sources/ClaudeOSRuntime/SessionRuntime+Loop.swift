...
extension SessionRuntime {
    ...
    private func runLoopBody(_ task: LoopTask) async {
        ...
        for attempt in 1...max(1, task.maxPasses) {
            ...
            appendLog(id, "▸ Deneme \(attempt) · üretici çalışıyor", .phase)

            // Maker. After the first cycle, hand back the checker's reason so it can fix.
            let makerPrompt = attempt == 1 ? task.prompt : """
                \(task.prompt)

                Bu işin önceki denemesi şu sebeple "tamamlanmadı" sayıldı:
                \(lastChecker)

                Bu geri bildirimi dikkate alarak işi tamamla.
                """
            ...
            appendLog(id, "▸ Deneme \(attempt) · denetçi kontrol ediyor", .phase)
        ...
        appendLog(id, "Deneme hakkı bitti, geçemedi.", .fail)
    ...
    private func runCheck(task: LoopTask, makerOutput: String) async -> (Bool, String) {
        ...
        case .agent:
            let gradePrompt = """
                Sen bağımsız ve şüpheci bir kalite denetçisisin. Aşağıdaki işin şu ölçütü KARŞILADIĞINI \
                doğrula; karşılamıyorsa bunu yakala.

                ÖLÇÜT (tamamlanma tanımı):
                \(task.doneWhen)

                ÜRETİLEN ÇIKTI:
                \(makerOutput)

                Kurallar: bu işi sen yapmadın, görevin onaylamak değil denetlemek. Ölçüt tam karşılanmıyorsa \
                ya da emin değilsen FAIL ver. Cevabının İLK satırı yalnızca PASS ya da FAIL olsun, ardından \
                1-3 cümlelik kısa gerekçe.
                """
    ...
    nonisolated static func runShellCheck(command: String, cwd: String, env: [String]) async -> (Bool, String) {
        await Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                return (false, "Komut çalıştırılamadı: \(error.localizedDescription)")
            }
        }.value
    }
}
