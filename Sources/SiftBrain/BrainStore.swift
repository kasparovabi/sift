...
public final class BrainStore: @unchecked Sendable {
    ...
    public init(path: String) throws {
        // WAL + busy timeout: the app process and the sift-brain-mcp child open
    ...
    /// Nodes and edges for the graph view, chosen together so the result is connected.
    ///
    /// Loading entities and relations with two independent limits produced a graph with no
    /// edges at all: the entity page (alphabetical) and the relation page (insertion order)
    /// referenced almost disjoint sets — on a real store, 0 of 800 loaded relations had both
    /// endpoints among the 300 loaded entities. Picking the most-connected entities first and
    /// then keeping only the relations *between* them makes an edgeless result impossible.
    public func graph(nodeLimit: Int) async throws -> (entities: [Entity],
                                                       relations: [(from: String, predicate: String, to: String)]) {
        try await dbQueue.read { db in
            let ranked = try String.fetchAll(db, sql: """
                SELECT id FROM (
                    SELECT subjectId AS id, COUNT(*) AS c FROM relation
                     WHERE invalidAt IS NULL AND subjectId <> objectId GROUP BY subjectId
                    UNION ALL
                    SELECT objectId AS id, COUNT(*) AS c FROM relation
                     WHERE invalidAt IS NULL AND subjectId <> objectId GROUP BY objectId
                ) GROUP BY id ORDER BY SUM(c) DESC LIMIT ?
                """, arguments: [nodeLimit])
            guard !ranked.isEmpty else { return ([], []) }

            let marks = { (n: Int) in Array(repeating: "?", count: n).joined(separator: ",") }
            let entities = try Entity.fetchAll(
                db, sql: "SELECT * FROM entity WHERE id IN (\(marks(ranked.count)))",
                arguments: StatementArguments(ranked))
            // A relation can outlive the entity row it points at, so edges are matched
            // against the entities actually fetched rather than the ranked id list.
            let present = Set(entities.map(\.id))
            let kept = ranked.filter { present.contains($0) }
            guard !kept.isEmpty else { return (entities, []) }

            let relations = try Row.fetchAll(db, sql: """
                SELECT subjectId, predicate, objectId FROM relation
                 WHERE invalidAt IS NULL AND subjectId <> objectId
                   AND subjectId IN (\(marks(kept.count))) AND objectId IN (\(marks(kept.count)))
                """, arguments: StatementArguments(kept + kept))
                .map { (from: $0["subjectId"] as String,
                        predicate: $0["predicate"] as String,
                        to: $0["objectId"] as String) }
    ...
    /// off the main thread in the separate sift-brain-mcp process, so it keeps
