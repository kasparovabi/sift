...
private struct SelectedSessionView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Text(session.displayTitle)
                        .font(Wasteland.font(20, weight: .semibold))
                    ...
                    if let cwd = session.cwd, !cwd.isEmpty {
                        Label(cwd, systemImage: "folder")
                            .font(.callout).foregroundStyle(Wasteland.textDim)
            ...
            HStack(spacing: 8) {
                ...
                iconButton("pencil", "Adı ve etiketleri düzenle") { editing = true }
                iconButton(copied ? "checkmark" : "doc.on.doc", "Sürdürme komutunu kopyala") {
                    copyResumeCommand()
                }
                if session.cwd != nil {
                    iconButton("folder", "Dizini Finder'da aç") { revealCwd() }
                }
                Spacer()
                Button { open() } label: {
    ...
    @ViewBuilder private var conversationPreview: some View {
        Divider()
        Text("Son konuşma").font(.caption).foregroundStyle(Wasteland.textDim)
        if !loaded {
            HStack(spacing: 6) {
                ProgressView().controlSize(.small)
                Text("Yükleniyor…").font(.caption).foregroundStyle(Wasteland.textDim)
            }
        } else if turns.isEmpty {
            if let message = session.firstMessage, !message.isEmpty {
                Text(message).font(.callout).foregroundStyle(Wasteland.textPrimary).textSelection(.enabled)
            } else {
                Text("Gösterilecek mesaj yok").font(.caption).foregroundStyle(Wasteland.textDim)
            }
        } else {
    ...
    private func turnRow(_ turn: TranscriptTurn) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 3) {
            ...
            Text(turn.text)
                .font(.caption)
                .lineLimit(4)
                .foregroundStyle(turn.role == .tool ? Wasteland.textDim : Wasteland.textPrimary)
                .textSelection(.enabled)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(9)
    ...
    private func roleStyle(_ role: TranscriptTurn.Role) -> (String, Color) {
        switch role {
        case .user: return ("Sen", Wasteland.cyan)
        case .assistant: return ("Claude", Wasteland.accent)
        case .tool: return ("Araç", Wasteland.acid)
        }
    }
    ...
    private var projectChip: some View {
        ...
        return HStack(spacing: 6) {
            ...
            Text(folder.isEmpty || folder == "/" ? "Proje" : folder)
...
private struct WelcomePane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 4) {
                Text(greeting)
                    .font(Wasteland.font(28, weight: .bold))
                    .foregroundStyle(Wasteland.accent)
                Text("\(index.totalSessionCount) oturum · \(index.projects.count) proje")
                    .foregroundStyle(Wasteland.textDim)
            }
            VStack(spacing: 8) {
                chip("\(last14.last ?? 0)", "bugün", "sun.max.fill", Wasteland.acid)
                chip("\(runtime.runningCount)", "çalışan", "bolt.fill", Wasteland.accent)
                chip("\(index.projects.count)", "proje", "folder.fill", Wasteland.cyan)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text("Son 14 gün").font(.caption).foregroundStyle(Wasteland.textDim)
                Sparkline(values: last14).frame(height: 46)
            }
            if let last = recents.first {
                VStack(alignment: .leading, spacing: 8) {
                    Button { resume(last) } label: {
                        Label("Kaldığın yerden devam et", systemImage: "play.fill")
                            .frame(maxWidth: .infinity)
                    }
                    .buttonStyle(.borderedProminent).controlSize(.large)
                    Text(last.displayTitle)
                        .font(.caption).foregroundStyle(Wasteland.textDim).lineLimit(1)

                    // A few more recent sessions as one-tap launchpad rows.
                    if recents.count > 1 {
                        Text("Diğer son oturumlar").font(.caption).foregroundStyle(Wasteland.textDim)
                            .padding(.top, 2)
            ...
            Button { newSession() } label: {
                Label("Yeni oturum başlat", systemImage: "plus").frame(maxWidth: .infinity)
            }
            .buttonStyle(.bordered).controlSize(.large)
            Text("İpucu: ⌥Space ile her yerden hızlı arama yapabilirsin")
                .font(.caption).foregroundStyle(Wasteland.textDim)
            Spacer()
        }
        .padding(24)
    ...
    private func recentRow(_ session: SessionSummary) -> some View {
        HStack(spacing: 8) {
            ...
            Text(session.displayTitle).font(.callout).foregroundStyle(Wasteland.textPrimary).lineLimit(1)
            Spacer(minLength: 4)
            if let date = session.lastActivity {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Wasteland.surface, in: RoundedRectangle(cornerRadius: 7))
    ...
    private func chip(_ value: String, _ label: String, _ icon: String, _ color: Color) -> some View {
        HStack(spacing: 10) {
            ...
            Text(value).font(Wasteland.font(16, weight: .semibold)).foregroundStyle(Wasteland.textPrimary)
            Text(label).font(.caption).foregroundStyle(Wasteland.textDim).lineLimit(1)
            Spacer(minLength: 0)
        }
        .padding(.horizontal, 12).padding(.vertical, 8)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Wasteland.surface, in: RoundedRectangle(cornerRadius: 10))
    }
            ...
            HStack(alignment: .bottom, spacing: 3) {
                ForEach(values.indices, id: \.self) { i in
                    RoundedRectangle(cornerRadius: 2)
                        .fill(Wasteland.accent.opacity(values[i] == 0 ? 0.15 : 0.85))
