...
struct GraphLayoutEngine {
    ...
    func relax(_ pos: inout [String: CGPoint], vel: inout [String: CGVector], size: CGSize,
        ...
        // Both force laws are unbounded at their extremes: the link spring grows with the
        // square of the distance, and repulsion diverges as two nodes converge. Dragging a
        // node a few hundred points away therefore produced forces in the thousands, every
        // connected node moved at the per-frame cap, overshot, and was yanked back — the
        // whole graph shook instead of the neighbours following. Clamping the magnitude
        // keeps the same shape of layout while making the sim impossible to detonate.
        let maxForce = k * 2
        ...
        for e in edges where e.from != e.to {
            ...
            let f = min(attraction * (dist * dist) / k, maxForce)
            let ux = dx / dist, uy = dy / dist
        ...
        resolveCollisions(&pos, vel: &vel, pinned: pinned, passes: 2)
        return moved / Double(n)
    }

    /// Positional collision resolution: nudge overlapping circles apart.
    ///
    /// The separation also cancels the velocity that drove the pair together. Without that,
    /// separating was purely positional while the spring that pushed them into each other
    /// stayed in the velocity: the next step drove them back, collision separated them
    /// again, and crowded nodes jittered forever instead of settling.
    private func resolveCollisions(_ pos: inout [String: CGPoint], vel: inout [String: CGVector],
                                   pinned: String?, passes: Int) {
        let n = ids.count
        /// Remove the component of `id`'s velocity heading along `(ux, uy)`, which is the
        /// direction it is being pushed away from — that component is what caused the overlap.
        func cancelApproach(_ id: String, _ ux: Double, _ uy: Double) {
            guard var v = vel[id] else { return }
            let along = v.dx * ux + v.dy * uy
            ...
            for a in 0..<n {
                ...
                for b in (a + 1)..<n {
                    ...
                    if ia == pinned {
                        ...
                        cancelApproach(ib, Double(-ux), Double(-uy))
                    } else if ib == pinned {
                        pos[ia] = CGPoint(x: pa.x + ux * overlap, y: pa.y + uy * overlap)
                        cancelApproach(ia, Double(ux), Double(uy))
                    } else {
