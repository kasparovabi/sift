...
public final class BrainViewModel {
    ...
    /// Entities that actually carry a relation, for the graph view. Kept apart from
    /// `entities` (which the list pages alphabetically) so the graph's nodes and its edges
    /// always come from the same query.
    public var graphEntities: [Entity] = []
    ...
    public func reload() async {
        ...
        // The graph is loaded as a graph: its nodes and the edges between them come from one
        // query, so the two can't disagree the way two independent limits did.
        let graph = try? await service.store.graph(nodeLimit: 140)
