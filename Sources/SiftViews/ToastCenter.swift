...
public final class ToastCenter {
    ...
    public func post(_ message: String, icon: String = "bell.fill",
                     tint: Color = Palette.accent, duration: Double = 4.2) {
    ...
    var body: some View {
        VStack(alignment: .trailing, spacing: 8) {
            ForEach(center.toasts) { toast in
                HStack(spacing: 11) {
                    Image(systemName: toast.icon).font(Palette.font(17)).foregroundStyle(toast.tint)
                    Text(toast.message).font(Palette.font(13, weight: .medium)).foregroundStyle(Palette.textPrimary).lineLimit(2)
                }
                ...
                .shadow(color: Palette.base.opacity(0.5), radius: 12, y: 5)
