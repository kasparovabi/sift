...
/// The directory name Claude Code stores a project under has every path separator turned
/// into a dash, so nested scratch folders look like unrelated top-level projects. Grouping
/// has to use the session's real cwd.
public sealed class ProjectGroupingTests
{
    private static readonly string[] Paths =
    {
        @"A:\harness",
        @"A:\harness\sandbox\clean-20260802\contact_email",
        @"A:\harness\sandbox\clean-20260802\a_drive_exe",
        @"A:\harness-other",
        @"A:\Obsidian\BEYIN",
    };

    [Fact]
    public void NestedScratchFoldersRollUpIntoTheProjectTheyAreIn()
    {
        var roots = IndexStore.Roots(Paths);
        Assert.Contains(@"A:\harness", roots);
        Assert.Contains(@"A:\Obsidian\BEYIN", roots);
        Assert.DoesNotContain(@"A:\harness\sandbox\clean-20260802\contact_email", roots);

        Assert.Equal(@"A:\harness",
            IndexStore.NearestRoot(@"A:\harness\sandbox\clean-20260802\a_drive_exe", roots));
    }

    [Fact]
    public void ASiblingWithASharedPrefixIsNotSwallowed()
    {
        var roots = IndexStore.Roots(Paths);
        Assert.Contains(@"A:\harness-other", roots);
        Assert.Equal(@"A:\harness-other", IndexStore.NearestRoot(@"A:\harness-other", roots));
        Assert.False(IndexStore.IsAncestor(@"A:\harness", @"A:\harness-other"));
        Assert.True(IndexStore.IsAncestor(@"A:\harness", @"A:\harness\sub"));
    }

    [Fact]
    public void TheDeepestMatchingProjectWins()
    {
        var roots = new HashSet<string>(new[] { @"A:\code", @"A:\code\app" },
                                        StringComparer.OrdinalIgnoreCase);
        Assert.Equal(@"A:\code\app", IndexStore.NearestRoot(@"A:\code\app\src", roots));
        Assert.Equal(@"A:\code", IndexStore.NearestRoot(@"A:\code\other", roots));
    }

    [Fact]
    public void ThePrefixPatternEndsInARealWildcard()
    {
        var pattern = IndexStore.LikePrefix(@"A:\harness");
        Assert.EndsWith(@"\\%", pattern);
        Assert.DoesNotMatch(@"\\%$", pattern[..^3]);
    }

    [Fact]
    public void WildcardsInsideAPathCannotWidenTheMatch()
    {
        var pattern = IndexStore.LikePrefix(@"A:\100%_done");
