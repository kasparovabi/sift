...
final class ExtractorTests: XCTestCase {
    ...
    func testParsesEnvelopeAndInnerJSON() throws {
        let inner = #"{"atoms":[{"t":"D","s":"use SwiftTerm","imp":8,"entities":["SwiftTerm"]}],"relations":[{"s":"sift","p":"uses","o":"SwiftTerm"}]}"#
        ...
        let result = try extractor.extract(transcript: "...", proj: "sift")
    ...
    func testLooksLikeExtractionDetectsOwnPrompt() {
        ...
        XCTAssertTrue(Extractor.looksLikeExtraction("...\n\(Extractor.instructionMarker) from the transcript below."))
        XCTAssertFalse(Extractor.looksLikeExtraction("a normal session where we wrote some code"))
    }
    ...
    func testDeletePersistedSessionRemovesOnlyMatchingTranscript() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent("sift-extract-\(UUID().uuidString)")
    ...
    func testDeletePersistedSessionNoOpWithoutSessionId() {
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("sift-missing-\(UUID().uuidString)")
