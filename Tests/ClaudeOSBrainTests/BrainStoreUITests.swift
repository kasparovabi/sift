...
final class BrainStoreUITests: XCTestCase {
    ...
    func testRecentAtomsOrdersByCreatedAtDesc() async throws {
        let s = try makeStore()
        ...
        let recent = try await s.recentAtoms(limit: 10)
        XCTAssertEqual(recent.first?.s, "new")
        XCTAssertEqual(recent.count, 2)
    }
    func testDeleteAtom() async throws {
        let s = try makeStore()
        let id = try s.insertAtom(t: .fact, s: "x", proj: "p", src: "x", imp: 5)
        try await s.deleteAtom(id: id)
        XCTAssertNil(try s.atom(id: id))
    }
    func testSetImportanceAndInvalidate() async throws {
        let s = try makeStore()
        let id = try s.insertAtom(t: .fact, s: "x", proj: "p", src: "x", imp: 5)
        try await s.setImportance(id: id, imp: 9)
        XCTAssertEqual(try s.atom(id: id)?.imp, 9)
        try await s.invalidate(id: id, at: 123)
        XCTAssertEqual(try s.atom(id: id)?.invalidAt, 123)
    }
    func testAllEntities() async throws {
        let s = try makeStore()
        _ = try s.resolveEntity(name: "SwiftTerm", kind: "lib")
        _ = try s.resolveEntity(name: "GRDB", kind: "lib")
        let count = try await s.allEntities(limit: 10).count
        XCTAssertEqual(count, 2)
    }
    func testAtomCount() async throws {
        let s = try makeStore()
        ...
        try await s.invalidate(id: id, at: 1)
