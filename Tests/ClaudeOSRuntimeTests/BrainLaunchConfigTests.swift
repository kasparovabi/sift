...
final class BrainLaunchConfigTests: XCTestCase {
    func testMCPConfigJSON() throws {
        let json = BrainLaunchConfig.mcpConfigJSON(binaryPath: "/Applications/Claude OS.app/Contents/MacOS/claudeos-brain-mcp",
                                                   dbPath: "/tmp/brain.sqlite")
        let obj = try JSONSerialization.jsonObject(with: Data(json.utf8)) as! [String: Any]
        let servers = obj["mcpServers"] as! [String: Any]
        let brain = servers["claudeos-brain"] as! [String: Any]
        XCTAssertEqual(brain["command"] as? String, "/Applications/Claude OS.app/Contents/MacOS/claudeos-brain-mcp")
        XCTAssertEqual((brain["env"] as? [String: Any])?["CLAUDEOS_BRAIN_DB"] as? String, "/tmp/brain.sqlite")
    }
